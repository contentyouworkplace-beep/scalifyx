  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && (window as any).fbq) {
      (window as any).fbq('track', 'CompleteRegistration', {
        content_name: 'Free Business Audit Booking',
        content_category: 'Special Audit',
      });
    }
  }, []);
  ...
  return (
    <div className="min-h-screen bg-[#FAFAF7] font-['Poppins',sans-serif] flex items-center justify-center px-4 py-16">
      <div className="max-w-lg w-full text-center">
        ...
        <h1 className="text-4xl font-extrabold text-[#0F172A] mb-3 tracking-tight">Your Audit Is Confirmed!</h1>
        <p className="text-[#6B7280] text-base mb-8 leading-relaxed">
          Your free audit is booked. You'll receive all details including the session link on WhatsApp.
        </p>
        ...
        <div className="bg-[#F0FDF4] border border-[#BBF7D0] rounded-2xl p-5 mb-8 text-left">
          ...
          <p className="text-[#16A34A] text-sm leading-relaxed">
            Our team will send you the session link, timing confirmation, and a short prep checklist on WhatsApp before your audit.
          </p>
        </div>
        ...
        <p className="mt-8 text-xs text-[#9CA3AF]">
          100% free — no payment, no card required.
        </p>

        <div className="mt-6">
          <Link href="/special-audit" className="text-xs text-[#16A34A] hover:underline">← Back to audit page</Link>
        </div>
      </div>
    </div>
  );
