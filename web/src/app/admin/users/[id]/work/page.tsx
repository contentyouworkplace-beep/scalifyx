...
export default function AdminUserWorkPage() {
  ...
  const load = useCallback(async () => {
    if (!userId) return;
    try {
      const data = await apiFetch(`/work/${userId}`);
      setTasks(data.tasks || []);
  ...
  const toggleTask = async (task: Task) => {
    ...
    try {
      await apiFetch(`/work/${userId}/complete`, {
        method: 'POST',
        body: JSON.stringify({ task_key: task.key, completed: !task.completed }),
      });
      await load();
    } catch {
  ...
  const submitComment = async (taskKey: string) => {
    ...
    try {
      await apiFetch(`/work/${userId}/comment`, {
        method: 'POST',
  ...
  const deleteComment = async (commentId: string, taskKey: string) => {
    try {
      await apiFetch(`/work/${userId}/comment/${commentId}`, { method: 'DELETE' });
      await load();
    } catch {
  ...
  return (
    <div className="max-w-2xl">
      {/* Back + header */}
      <div className="mb-6">
        <Link href="/admin/users" className="text-xs text-zinc-500 hover:text-white transition mb-3 inline-flex items-center gap-1">
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="15 18 9 12 15 6"/></svg>
          Back to Users
        </Link>
        <h1 className="text-2xl font-extrabold">{userName || 'User'}'s Work</h1>
        {businessName && <p className="text-sm text-zinc-500 mt-0.5">{businessName}</p>}
      </div>
                    ...
                    <div className="flex items-center gap-3 px-4 py-3.5">
                      ...
                      <button
                        ...
                        className={`w-5 h-5 rounded-full flex-shrink-0 border-2 flex items-center justify-center transition ${
                          task.completed
                            ? 'bg-primary border-primary hover:bg-primary/70'
                      ...
                      >
                        ...
                        {task.completed && task.completed_at && (
                          <div className="text-[11px] text-zinc-600 mt-0.5">Marked {timeAgo(task.completed_at)}</div>
                        )}
                      </button>
                              ...
                              <div key={c.id} className="flex gap-2.5 group">
                                ...
                                <div className="flex-1 min-w-0">
                                  <div className="flex items-center gap-1.5">
                                    ...
                                    <button
                                      onClick={() => deleteComment(c.id, task.key)}
                                      className="ml-auto text-[10px] text-zinc-700 hover:text-red-400 transition opacity-0 group-hover:opacity-100"
                                    >
                                      delete
                                    </button>
                                  </div>
