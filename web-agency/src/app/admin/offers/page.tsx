...
export default function AdminOffersPage() {
  ...
  const fetchOffers = useCallback(async () => {
    try {
      ...
      toast.error('Load error: ' + (e.message || 'unknown'));
    }
          ...
          {offers.map((offer) => {
            ...
            return (
              <div key={offer.id} className={`rounded-[14px] bg-surface border border-border p-4 ${!offer.is_active ? 'opacity-60' : ''}`}>
                ...
                {offer.razorpay_payment_link_url && (
                  <div className="flex items-center gap-2 my-2 px-3 py-2 rounded-lg bg-green-500/10 border border-green-500/20">
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="text-green-400 flex-shrink-0"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>
                    <a href={offer.razorpay_payment_link_url} target="_blank" rel="noopener noreferrer" className="text-xs text-green-400 font-medium truncate hover:underline">
                      {offer.razorpay_payment_link_url}
                    </a>
                    <button
                      onClick={() => { navigator.clipboard.writeText(offer.razorpay_payment_link_url!); toast.success('Link copied!'); }}
          ...
          <div className="bg-bg rounded-t-3xl md:rounded-2xl w-full max-w-md max-h-[90vh] overflow-y-auto p-5" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              ...
              <div><label className="text-xs font-semibold text-zinc-500 mb-1 block">Offer Name</label>
                <input className="w-full px-3.5 py-3 bg-surface border border-border rounded-xl text-sm text-white placeholder-zinc-600 focus:outline-none focus:border-primary" placeholder="e.g. Scalify Pro, 7-Day Free Trial" value={form.name} onChange={(e) => setForm(f => ({...f, name: e.target.value}))} /></div>
